...
#select only eukaryotes
mtags_tax <- mtags_tax %>%
  dplyr::filter(domain == "Eukaryota"|genus %in% c("Unassigned", "Unaligned"))
...
write_rds(pcoa, "data/mtags_eukaryotes_exp2_pcoa_ape_output.rds")
...
pcoa_plot_dat <- pcoa_plot_dat %>%
  dplyr::mutate(domain = rep("Eukaryota", nrow(pcoa_plot_dat)))
readr::write_rds(pcoa_plot_dat, "data/mtags_eukaryotes_exp2_pcoa.rds")
#plot
...
#save plot
ggplot2::ggsave("output/figures/figureS2.jpg",
                pcoa_plot,
                device = "jpg",
                dpi = 300,
                units = "cm",
