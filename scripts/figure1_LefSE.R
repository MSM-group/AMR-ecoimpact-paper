...
library(pacman)
pacman::p_load("tidyverse", "readxl", "vegan", "ggpubr","janitor", "ape", "edgeR", "phyloseq", "mia", "lefser", "phyloseqCompanion", "microeco", "file2meco", "ggtree", "RColorBrewer")
...
ggtree::ggtree(clades_dat, layout = "circular", aes(color = phylum)) +
  ...
  ggplot2::scale_fill_manual(values = c("#93D5E7", "#EDC899", "#CFAD8D", "white"), guide = guide_legend(label.theme = element_text(size = 18))) +
  ggplot2::scale_color_manual(values = getPalette(n_colors), guide = guide_legend(label.theme = element_text(face = "italic", size = 18))) +
  ggplot2::theme(legend.title = element_text(size = 20))
ggsave("output/figures/figure1_LefSE.jpg",
       dpi = 300,
       device = "jpeg",
       units = "cm",
       width = 75/2,
